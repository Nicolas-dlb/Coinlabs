...
import Graph from "./StatsGraph/StatsGraph";

type StatsProps = {
  search?: true | undefined;
};
function Stats({ search = undefined }: StatsProps) {
    ...
    if (
      ...
      setData(
        [...Array(length).keys()].map((x) => ({
          ...
          timestamp: history[bitcoin].slice(l)[x]?.timestamp,
        }))
      );
    } else {
      setData(
