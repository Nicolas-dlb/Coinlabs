...
import { selectBalanceHistory, selectWallets, setBalanceHistory, setExpensesHistory, setIncomeHistory, setWalletHistory, setWallets } from 'redux/reducers/walletsSlice';
...
const length = active === "Month" ? 601 : active === "Week" ? 161 : active === "Year" ? 361: active === "3Month" ? 81 : active === "6Month" ? 181 : 0 ;
 const l = -length;
useEffect(() => {
    ...
    if (Number(nbr) === 0 && Number(nbrCurrency) === 0) {
      ...
      dispatch(
        setWallets({
          ...
          usd:
            wallet.usd +
            Number(getNumberFixed(numberCrypto, 2)),
      ...
      dispatch(
        setWalletHistory({
         status:  'Sell',
         name: cryptoSelected.charAt(0).toUpperCase() + cryptoSelected.slice(1),
      ...
      if (
        Number(numberCurrency) > wallet.usd ||
        Number(numberCurrency) === 0
      ) {
        document
          .getElementById("inputBuy")!
          .classList.add("bad_input_send_crypto");
...
dispatch(setExpensesHistory({
    ...
    timestamp,
      ...
      dispatch(
        setWalletHistory({
          status: 'Buy',
          name: currencySelected.charAt(0).toUpperCase() + currencySelected.slice(1),
...
<div className="input exchange_element">
    <input className="exchange_element" onFocus={() => {
              $("#inputBuy").val(0);
              $("#inputBuy").css("opacity", "0");
              ...
              document
                ...
                .classList.remove("bad_input_send_crypto");
              handleChangeCrypto();
            }} min="0" type="number" id="inputSell" />
<SelectExchange setCryptoSelected={setCryptoSelected} items="sell" />
</div>
...
<div className="input exchange_element">
    <input className="exchange_element" onFocus={() => {
            ...
            }} type="number" id="inputBuy" />
    <SelectExchange setCurrencySelected={setCurrencySelected} items="buy" />
</div>
                </div>
                ...
                <button onClick={handleClick} className="btn-exchange exchange_element" type="button">Exchange</button>
            </div>
