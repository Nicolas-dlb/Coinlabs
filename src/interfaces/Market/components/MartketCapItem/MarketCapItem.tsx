...
import { selectMarket, selectVariation } from 'redux/reducers/marketSlice';
import { getNumberFixed, numberWithSpaces } from 'utils/utils';
import './MarketCapItem.scss';

type MarketCapItemProps = {
    name: string
}

function MarketCapItem({name}: MarketCapItemProps) {
    ...
    const volume = market[name.toLowerCase()]?.usd_market_cap;
 const color = variation[name.toLowerCase()].cap > 0 ? "#37bb77" : "#f1c94f";
    const angle = variation[name.toLowerCase()].cap > 0 ? "140deg" : "210deg";
    return (
        <div className="market_cap_item">
            <div className="item_name">
                {icon}
                <p>{name}</p>
            </div>
            <div style={{color: color}} className="item_variation">
                 <svg
                ...
                <p>{getNumberFixed(variation[name.toLowerCase()].cap, 0)}</p>
                <p>%</p>
            </div>
            <div className="item_volume">
                <p>$</p>
               <p>{numberWithSpaces(getNumberFixed(volume, 0)).split(" ")[0]} Milliards</p>
            </div>
        </div>
    )
}

export default MarketCapItem
